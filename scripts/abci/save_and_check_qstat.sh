...
# 各ジョブについて情報を取得
node_kinds=("rt_F" "rt_G.large" "rt_G.small" "rt_C.large" "rt_C.small" "rt_AF" "rt_AG.small")
while read -r job_id state; do
  ...
  if [ -z "$node_kind" ]; then
    hard_resource_list=$(echo "$job_info" | grep 'hard resource_list' | awk '{print $3}')

    node_kind="normal"
    for kind in "${node_kinds[@]}"; do
      if echo "$hard_resource_list" | grep -q "$kind"; then
        node_kind="$kind"
        break
      fi
    done
