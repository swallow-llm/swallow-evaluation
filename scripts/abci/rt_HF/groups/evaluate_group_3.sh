...
# 並列処理でスクリプトを実行
CUDA_VISIBLE_DEVICES=0,1,2,3 bash "$REPO_PATH/scripts/abci/rt_HF/evaluate_ja_xlsum.sh" \
...
CUDA_VISIBLE_DEVICES=4,5 bash "$REPO_PATH/scripts/abci/rt_HF/evaluate_english_gpqa.sh" \
    "$REPO_PATH" "$HUGGINGFACE_CACHE" "$MODEL_NAME_PATH" \
